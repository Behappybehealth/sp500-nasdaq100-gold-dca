...
>
> **2026-08-18 部分拍板（仅落点）**：用户在日志体系讨论中裁定运行日志落点 = 项目 `logs/` 目录（`*.log` 不入库，目录由 `.gitkeep` 占位）。⚠️ 该落点对本机/长期部署有效；Streamlit Cloud 容器重启即失，实现时仍需另配持久化或外部探针。修复路径（埋点范围、探针选型）仍待 C 批单独确认——本条目状态维持 🔴。
